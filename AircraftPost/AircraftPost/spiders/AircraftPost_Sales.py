############3       AircraftPost_Sales

import scrapy
from datetime import datetime

class AircraftPost_Sales(scrapy.Spider):
    name = 'AircraftPost_Sales'
    # url = "https://www.aircraftpost.com/recent_sales/aircraft_sales"
    url = "https://aircraftpost.com/recent_sales/aircraft_sales"
    headers = {
    ...
    # make_model_url = "https://www.aircraftpost.com/recent_sales/detail_sales?make_model={}&period=12"
    make_model_url = "https://aircraftpost.com/recent_sales/detail_sales?make_model={}&period=12"
    make_model_headers = {
          'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    ...
    custom_settings = {
        'FEED_URI': f'output/AircraftPost_Sales {datetime.now().strftime("%d-%m-%Y %H-%M-%S")}.csv',
        'FEED_FORMAT': 'csv',
    ...
    def parse(self, response):
        for make_model in response.xpath("//select[contains(@name,'make_model')]/option"):
            make_model_ID = make_model.css("::attr(value)").get('')
            make_model_Name = make_model.css("::text").get('')
            detail_url = self.make_model_url.format(make_model_ID)
            print(make_model_ID, '   ||   ', make_model_Name, '||', detail_url)
            if make_model_Name is not '':
    ...
    def details(self, response):
        ...
        for data in response.xpath("//*[contains(@class,'center th-sub-title')]"):
            no += 1
            item['ER'] = 'Yes' if response.css(f'#row_type_significant_177 td:nth-child({no}) ::text').get('') == '✔' else ' '
            item['Serial Number'] = data.css('label::text').get('')
            item['Previous Registration'] = response.css(f'#row_prev_registration td:nth-child({no}) ::text').get('').strip()
            ...
            item['Current Airframe Hours'] = '{:,}'.format(int(response.css(f'#row_air_frame_hours td:nth-child({no}) ::text').get('')))
            item['Previous Country'] = response.css(f'#row_prev_country_code td:nth-child({no}) span::text').get('').strip()
            item['Days On Market'] = response.css(f'#row_days_on_market td:nth-child({no}) ::text').get('')
            item['Ask Price (M)'], item['Ask Price_MO'] = '', ''
            if response.css(f'#row_asking_price td:nth-child({no}) ::text').get('').strip() == 'M/O':
                item['Ask Price_MO'] = 'Yes'
            else:
                item['Ask Price (M)'] = ''.join(e.strip().replace('$','').replace('TBD','').replace('Lease','') for e in response.css(f'#row_asking_price td:nth-child({no}) ::text').getall())
            item['Sale Price'], item['Sale Price_TBD'] = '', ''
            if response.css(f'#row_sale_price td:nth-child({no}) ::text').get('').strip() == 'TBD':
                item['Sale Price_TBD'] = 'Yes'
            else:
                # item['Sale Price (M)'] = response.css(f'#row_sale_price td:nth-child({no}) ::text').get('').strip().replace('$','').replace('TBD','').replace('Lease','')
                item['Sale Price'] = ''.join(e.strip().replace('$','').replace('TBD','').replace('Lease','') for e in response.css(f'#row_sale_price td:nth-child({no}) ::text').getall())
            date_str = response.css(f'#row_sale_date td:nth-child({no}) ::text').get('').strip()
            date_object = datetime.strptime(date_str, '%m-%d-%y')
            item['Sale Date'] = date_object.strftime('%d/%m/%Y')
            item['Engine Program Type'] = response.css(f'#row_engine_program td:nth-child({no}) a::text').get('').strip()
            ...
            # item['Country'] = response.css(f'#row_country td:nth-child({no}) ::text').get('').strip()
            ...
            # item['Days On Market'] = response.css(f'#row_days_on_market td:nth-child({no}) ::text').get('')
            #
            # item['Engine Hours to Inspection'] = response.css(f'#row_engine_hours td:nth-child({no}) ::text').get('').strip()
            ...
            # item['Current Airframe Hours'] = '{:,}'.format(int(response.css(f'#row_air_frame_hours td:nth-child({no}) ::text').get('')))
            # item['Current Total Landings'] = '{:,}'.format(int(response.css(f'#row_total_landings td:nth-child({no}) ::text').get('')))
            ...
            # item['Certification'] = response.css(f'#row_certification td:nth-child({no}) ::text').get('')
            ...
            #
            # item['Engine Hours to Inspection'] = response.css(f'#row_engine_hours_to_inspection td:nth-child({no}) ::text').get('')
            # item['Galley'] = response.css(f'#row_galley td:nth-child({no}) ::text').get('')
            # item['Lav'] = response.css(f'#row_lav td:nth-child({no}) ::text').get('')
            # item['Engine Inspection Date'] = response.css(f'#row_engine_inspection_due td:nth-child({no}) ::text').get('')
            # # # item['Airframe Inspection (60 month)'] = '01-' + response.css(f'#row_airframe_inspection_due td:nth-child({no}) ::text').get('')
