...
class FightOdds_IO(scrapy.Spider):
    name = 'FightOdds_IO'
    ...
    data1 = json.dumps({
      "query": "query EventOfferTableQrQuery(\n  $eventPk: Int!\n  $isCancelled: Boolean\n) {\n  sportsbooks: allSportsbooks(hasOdds: true) {\n    ...EventOfferTable_sportsbooks\n  }\n  eventOfferTable(pk: $eventPk, isCancelled: $isCancelled) {\n    slug\n    pk\n    name\n    ...EventOfferTable_eventOfferTable\n    id\n  }\n}\n\nfragment EventOfferTable_eventOfferTable on EventOfferTableNode {\n  name\n  pk\n  fightOffers {\n    edges {\n      node {\n        id\n        fighter1 {\n          firstName\n          lastName\n          slug\n          id\n        }\n        fighter2 {\n          firstName\n          lastName\n          slug\n          id\n        }\n        bestOdds1\n        bestOdds2\n        slug\n        propCount\n        isCancelled\n        straightOffers {\n          edges {\n            node {\n              sportsbook {\n                id\n                shortName\n                slug\n              }\n              outcome1 {\n                id\n                odds\n                ...OddsWithArrowButton_outcome\n              }\n              outcome2 {\n                id\n                odds\n                ...OddsWithArrowButton_outcome\n              }\n              id\n            }\n          }\n        }\n      }\n    }\n  }\n}\n\nfragment EventOfferTable_sportsbooks on SportsbookNodeConnection {\n  edges {\n    node {\n      id\n      shortName\n      slug\n    }\n  }\n}\n\nfragment OddsWithArrowButton_outcome on OutcomeNode {\n  id\n  ...OddsWithArrow_outcome\n}\n\nfragment OddsWithArrow_outcome on OutcomeNode {\n  odds\n  oddsPrev\n}\n",
      "variables": {        "eventPk": 5416,        "isCancelled": False        }
    })
    data2 = json.dumps({
    ...
    custom_settings = {
        'FEED_URI': 'Output/FightOdds Record - Complete.csv',
        'FEED_FORMAT': 'csv',
    ...
    def parse(self, response):
        ...
        count = 1
        for events in data['data']['upcomingEvents']['edges']:#[7:20]:
            event_pk = events.get('node').get('pk')
            event_name = events.get('node').get('name')
            print(count, event_pk, event_name)
            count += 1

            '''         Extracting further data                 '''
            payload1 = json.loads(self.data1)
            payload1['variables'] = {"eventPk": event_pk}
            payload = json.dumps(payload1)
            yield scrapy.Request(url=self.url, method='POST', body=payload, headers=self.headers,
                                 callback=self.listing_parse, meta={'event_name':event_name,})

    def listing_parse(self, response):
        print(' ### LISTING PAGE ### ')
        data = json.loads(response.text)
        event = response.meta.get('event_name')
        for Matches in data['data']['eventOfferTable']['fightOffers']['edges']:
            item = {"Event": event, "Fight": None}
            ...
            payload = json.dumps(payload1)
            yield scrapy.Request(url=self.url, method='POST', body=payload, headers=self.headers,
                                 callback=self.details_parse, meta={'event': event, 'fight': fight})
    ...
    def details_parse(self, response):
        ...
        for BET in data['data']['fightPropOfferTable']['propOffers']['edges']:
